...
public class ToolRackBER implements BlockEntityRenderer<ToolRackBlockEntity, MultiItemContainerRenderState> {
    ...
    @Override
    public void extractRenderState(ToolRackBlockEntity blockEntity, MultiItemContainerRenderState state, float partialTicks, Vec3 cameraPosition, ModelFeatureRenderer.@Nullable CrumblingOverlay breakProgress) {
    ...
    public void submit(MultiItemContainerRenderState state, PoseStack stack, SubmitNodeCollector collector, CameraRenderState camera) {
        ...
        for (int i = 0; i < 2; i++) {
            for (int j = 0; j < 2; j++) {
                ItemStackRenderState item = state.items[i * 2 + j];
                if (item == null) {
                    continue;
                }
                stack.pushPose();
