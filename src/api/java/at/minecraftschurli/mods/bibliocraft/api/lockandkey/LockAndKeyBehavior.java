package at.minecraftschurli.mods.bibliocraft.api.lockandkey;
...
public interface LockAndKeyBehavior<T extends BlockEntity> {
    ...
    /// @return the [BlockEntity]'s [LockCode].
    LockCode getLockKey(T blockEntity);

    /// Sets the [BlockEntity]'s [LockCode].
    ///
    /// @param blockEntity the [BlockEntity]
    /// @param lock        the [LockCode] to set
    void setLockKey(T blockEntity, LockCode lock);

    /// Returns the [BlockEntity]'s display name.
    ///
    /// @param blockEntity the [BlockEntity]
    /// @return the [BlockEntity]'s display name.
    Component getDisplayName(T blockEntity);

    record Simple<T extends BlockEntity>(Function<T, LockCode> lockGetter, BiConsumer<T, LockCode> lockSetter, Function<T, Component> nameGetter) implements LockAndKeyBehavior<T> {
        @Override
